...
import { Grid, Card, Text, Box, Layout, MyLoader, Select, Button } from '../components';
...
const Templates = (props) => {
    ...
    function handleFilterByCategory(e) {
        ...
        const filteredData = res.filter (data => {
            return data.category.includes(e.target.value)
        });
    ...
    function filterAlphabetically(e) {
        ...
        if (e.target.value === 'Default') {
             sortedData = templateData
        }

        if (e.target.value === 'Ascending') {
             sortedData = templateData?.sort((a,b) => {
    ...
    return (
        <Layout>
            <Box className='header' display='grid' style={{gridTemplateColumns: '55% 45%'}} >
               <input 
                 type='search' 
                 placeholder='Search Templates' 
                 onKeyUp={e => handleSearchTemplates(e)}
                />
                <Box className='select' display="flex"  style={{alignItems: 'center'}} justifyContent='space-between'>
            ...
            <Box>
                {
                    isLoading ? (
                        ...
                        <Box>
                            <Grid default='repeat(3, 1fr)' pad='1.875rem 0' gap='3.125rem'>
                                {
                                    paginatedData && paginatedData.map((template, index) => {
                                        return (
                                            <Card
                                                key={index}
                                            >
                                                <Text pad='0.625rem 1.25rem' fontSize='1.5rem'>{capitalizeFirstLetter(template?.name)}</Text>
                                                <Text pad='0 1.25rem'>{capitalizeFirstLetter(template?.description)}</Text>
                                                <Box bg='#F9F9F9' height='2.5rem'>
                                                    <Text pad='0.625rem 1.25rem'>
                                                        <a href={template?.link} rel='noreferrer noopener' target='_blank'>Use Template</a>
                                                    </Text>
                                                </Box>
                                            </Card>
                                        )
                            ...
                            <Box display='flex' justifyContent='space-between' fontSize='1.125rem'>
                                <Button 
                                    clear
